...
void gmFoamTranslator::add_solid_fluid_interface()
{
	...
	//##################### adding this section to rename ESSI interface nodes information file as ESSI_nodes.fei and delete ESSI surface information file generated by gmessi################################################
	string rm_file_dir=getFilePath()+"/"+s+"_ESSI_Simulation/*_Elements.txt";
	string mv_file_dir=getFilePath()+"/"+s+"_ESSI_Simulation/*_Nodes.txt ";
	string mved_file_dir=getFilePath()+"/"+s+"_ESSI_Simulation/essi_nodes.fei";
	string seded_file_dir=getFilePath()+"/"+s+"_ESSI_Simulation/ESSI_nodes.fei";

	string rm_str="rm "+rm_file_dir;
	string mv_str="mv "+mv_file_dir+mved_file_dir;

	string sed_str="sed 's/[\t]/ /g' "+mved_file_dir+" > "+seded_file_dir;

	string rm_essi_nodes="rm "+mved_file_dir;

	system(rm_str.c_str());
	system(mv_str.c_str());
	system(sed_str.c_str());
	system(rm_essi_nodes.c_str());
